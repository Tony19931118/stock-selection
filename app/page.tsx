...
export default function Home() {
  ...
  const results = useMemo(() => stocks
    ...
    .sort((a, b) => b.difference - a.difference), [query, stocks]);

  const exportExcel = () => {
    const header = "股票代號,股票名稱,即時價格,半年最高價,半年最低價,中間值,差異百分比,本益比,殖利率,報價時間\n";
    const rows = results.map((stock) =>
      [stock.code, stock.name, stock.price, stock.high, stock.low, stock.midpoint, `${stock.difference.toFixed(2)}%`, stock.peRatio?.toFixed(2) ?? "-", stock.dividendYield === null ? "-" : `${(stock.dividendYield * 100).toFixed(2)}%`, stock.quoteTime].join(","),
    ).join("\n");
    ...
    <main className="shell">
      ...
      <section className="content">
        ...
        <section className="results-panel" id="results">
          ...
          <div className="table-wrap"><table><thead><tr><th>股票</th><th>即時成交價</th><th>半年高點</th><th>半年低點</th><th>中間值</th><th>差異百分比 <span className="sort-arrow">↓</span></th><th>本益比</th><th>殖利率</th><th>報價時間</th></tr></thead><tbody>{results.map((stock) => <tr key={stock.code}><td><div className="stock-name"><strong>{stock.code}</strong><span>{stock.name}</span></div></td><td className="price">{stock.price.toLocaleString("zh-TW")}</td><td>{stock.high.toLocaleString("zh-TW")}</td><td>{stock.low.toLocaleString("zh-TW")}</td><td>{stock.midpoint.toLocaleString("zh-TW")}</td><td><span className="difference">-{stock.difference.toFixed(2)}%</span></td><td>{stock.peRatio?.toFixed(2) ?? "-"}</td><td>{stock.dividendYield === null ? "-" : `${(stock.dividendYield * 100).toFixed(2)}%`}</td><td className="quote-time"><i />{stock.quoteTime}</td></tr>)}</tbody></table>{results.length === 0 && <div className="empty-state">查無符合條件的股票</div>}</div>
